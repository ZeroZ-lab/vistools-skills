...
[vistools](https://github.com/ZeroZ-lab/vistools) 的 Claude Code 插件 — 让 AI 编程助手具备检查、导航、裁剪、采样、比较图片、衡量大图焦点分布，以及估算白平衡偏色的能力，所有输出都是结构化 JSON，附带指回源图的坐标映射。
...
5. 在视觉回归任务中比较 expected / actual 图片
6. 在图像质量相关任务中估算焦点或白平衡
7. 报告发现并附源图坐标
...
| `sample` | 点/区域取色器（只读） |
| `diff` | expected / actual 图片的像素差异统计 |
...
`focus-map` 需要 CLI `v0.2.4+`；`white-balance` 需要 CLI `v0.2.5+`；`diff` 需要 CLI `v0.2.6+`。
