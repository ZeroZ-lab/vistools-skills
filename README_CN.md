...
[vistools](https://github.com/ZeroZ-lab/vistools) 的 Claude Code 插件 — 让 AI 编程助手具备检查、导航、裁剪、采样，以及衡量大图焦点分布的能力，所有输出都是结构化 JSON，附带指回源图的坐标映射。
...
`focus-map` 需要 CLI `v0.2.3+`。源码仓库已经包含该命令，插件仓库内置二进制可能要等下一次多平台刷新后才全部带上。
